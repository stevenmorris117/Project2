...
Floater.prototype.act = function(step) {
  var wobbleSpeed = 10, wobbleDist = 0.7;
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Portal.prototype.act = function(step) {
  var wobbleSpeed = 7.5, wobbleDist = 0.2;
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    // If there aren't any floaters left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "floater";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
